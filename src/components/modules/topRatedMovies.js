...
import { getTopRatedMoviesAction } from "../../services/actions";
...
class TopRatedMoviesList extends Component {
  componentDidMount() {
    this.props.getTopRatedMoviesAction();
  }

  movieListRender() {
    console.log(this.props);
    if (!this.props.topRated.length) {
      return <div className="noMovies">No Movies Found</div>;
    }

    return this.props.topRated.map((x) => {
      return (
...
export default connect(mapStateToProps, { getTopRatedMoviesAction })(
  TopRatedMoviesList
);
