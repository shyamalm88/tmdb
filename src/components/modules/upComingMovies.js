...
import { getUpComingMoviesAction } from "../../services/actions";
...
class UpComingMoviesList extends Component {
  componentDidMount() {
    this.props.getUpComingMoviesAction();
  }

  movieListRender() {
    console.log(this.props);
    if (!this.props.upComing.length) {
      return <div className="noMovies">No Movies Found</div>;
    }

    return this.props.upComing.map((x) => {
      return (
...
export default connect(mapStateToProps, { getUpComingMoviesAction })(
  UpComingMoviesList
);
